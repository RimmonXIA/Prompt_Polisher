...
- **`sample_report.json`** — Shape resembles envelope `data`; **`radar_analysis` and `routing_decision` keys vary** with the live model and prompts. Do not treat every key in the sample as stable for integration tests without pinning the model and input.
