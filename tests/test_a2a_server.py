...
def test_send_message_missing_input() -> None:
...
def test_get_task_not_found() -> None:
    payload = {"jsonrpc": "2.0", "method": "GetTask", "id": 4, "params": {"id": "unknown"}}
    response = client.post("/a2a/v1", json=payload)
    assert response.status_code == 200
    assert "error" in response.json()
...
def test_send_message_success_mock(monkeypatch: Any) -> None:
