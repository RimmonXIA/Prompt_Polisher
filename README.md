...
**TL;DR (English).** **Python 3.11+ CLI**: a **LangGraph** multi-node **prompt compiler**—radar → optional **threat gate** → routing → structured compile → **critic** loop—plus blueprint and DSPy-style sketches; use **`--envelope`** for agents and automation; **OpenAI**- or **DeepSeek**-compatible backends via `LLM_PROVIDER`.

**一句话（中文）。** **Python 命令行工具**（3.11+）：基于 **LangGraph** 的多节点**提示词编译**流水线（意图雷达 → 可选威胁闸门 → 路由 → 编译 → **Critic**），支持 **`--envelope`** 供 Agent 与自动化消费；理论长文见下文链接。
...
| | |
| --- | --- |
| **EN** | Copy [`.env.example`](.env.example) to `.env`, set `OPENAI_API_KEY` or `DEEPSEEK_API_KEY` (and `LLM_PROVIDER` if needed). Run `uv sync --all-groups`, and `uv run prompt-polisher "Your raw task"` to print the compiled prompt. Use `-m` (or `--markdown`) for a readable compilation report, or `--envelope` for a versioned agent JSON envelope. |
| **中文** | 将 [`.env.example`](.env.example) 复制为 `.env`，填写 `OPENAI_API_KEY` 或 `DEEPSEEK_API_KEY` 等。执行 `uv sync --all-groups`，`uv run prompt-polisher "你的原始需求"` 输出最终 prompt；`-m` 输出可读编译报告，`--envelope` 输出带版本字段的 JSON 信封。 |

```bash
uv sync --all-groups
cp .env.example .env   # edit API keys and LLM_PROVIDER
uv run prompt-polisher "Summarize this repository for a release note."
uv run prompt-polisher -m "Your raw requirement"
uv run prompt-polisher --envelope "Your raw requirement"
```

**Examples.** Redacted / synthetic inputs and report shapes (`-m` / `--envelope`) live under [examples/](examples/).
...
```bash
# 从参数传入原始需求，打印最终 prompt (缺省标准管道)
uv run prompt-polisher "你的原始需求"

# 人类可读的 Markdown 编译报告（含雷达/路由/Critic/交付物/前置输入对照）
uv run prompt-polisher -m "你的原始需求"

# 版本化 JSON 信封（供 Agent / 工具协议使用）
...
For scripts, CI, and coding agents, treat the CLI as a small **tool protocol**: **stdout** carries the primary payload; **logging** goes to **stderr** (see [`src/prompt_polisher/logging_config.py`](src/prompt_polisher/logging_config.py)). Prefer **`--envelope`** when you need structured radar / gate / critic context, not only the final prompt string.

**Exit codes** (stable for automation):

| Code | Meaning |
| --- | --- |
| `0` | Run finished and the threat gate did **not** abort compilation (`compilation_aborted` is false). Applies to every output mode (default text, `-m`, `--envelope`). |
...
- `PROMPT_POLISHER_AGENT`: if set to a truthy value (`1`, `true`, `yes`, `on`) and you do **not** pass `-m` or `--envelope`, the CLI prints **`--envelope`** JSON on stdout (same exit semantics as above).
- `LOG_LEVEL` / `LOG_JSON`: see [`.env.example`](.env.example); reduce noise with `--quiet` (stderr shows warnings and errors only).
- **Structured JSON to stdout** (`--envelope` or `PROMPT_POLISHER_AGENT` defaulting to envelope): `httpx` and `httpcore` INFO lines on stderr are raised to WARNING so request spam does not drown out your payload; use **`--verbose`** to restore those HTTP logs; use **`--quiet`** to also drop most `prompt_polisher` INFO on stderr.

**Envelope shape** (`--envelope`): top-level keys `ok`, `schemaVersion` (currently `1`), `data` (compilation report), and `error` (`null` on success, or an object with `code`, `message`, `detail` when the gate aborts). Bump `schemaVersion` only when intentionally breaking the envelope layout.
...
**LLM-shaped JSON:** In envelope `data`, `radar_analysis` and `routing_decision` are **model-produced objects**; **key sets are not stable** across models or prompts. For automation, rely on `summary`, `deliverables`, and (when present) envelope `error`; treat other keys inside those objects as **optional extensions**.
...
**中文简述：** 自动化时请用 **stdout 解析结果、stderr 看日志**；需要结构化上下文时排他性地使用 **`--envelope`**；用 **退出码 0 / 2 / 1** 区分成功编译、闸门中止与错误；信封字段 **`ok`** 仅表示**未因威胁闸门中止**，不等于任务或模型输出成功；**`radar_analysis` / `routing_decision` 内键名不保证稳定**。设置 **`PROMPT_POLISHER_AGENT=1`** 可在未指定其它输出格式时默认输出信封 JSON；JSON 主输出模式下默认压低 **`httpx`/`httpcore`** 的 INFO，需要排障时用 **`--verbose`**，需要更少本仓库日志时用 **`--quiet`**。
